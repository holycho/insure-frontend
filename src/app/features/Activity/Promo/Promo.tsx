...
import { Promo as PromoItem } from 'app/bff/models/promo/list';
import { SortPageReq } from 'app/bff/models/base';
import { useDispatch } from 'react-redux';
import { fetchPromoDetailAction, initialPromoAction } from 'app/store/activity/actions';
...
const Promo: React.FC = () => {
  ...
  const promoState = useSelector((state: RootState) => state.activity.promo);
  const [page, setPage] = useState<number>(1);
  const totalPage = Math.ceil(promoState.totalCount / PAGE_SIZE);
  ...
  useEffect(() => {
    ...
    reduxDispatch(initialPromoAction(args));
   ...
   * @description 導頁至明細頁
   */
  const handleRedirect = (data: PromoItem) => {
      ...
      <div className="inside-page-00-showcase">
        <div className="inside-page-00-showcase__former">
          <div className="inside-page-00-banner">
            <div className="inside-page-00-banner__bg" style={{ backgroundImage: `url(${require('assets/img/top-promo-banner.png')})` }} />
            <div className="inside-page-00-banner__inner">
              <div className="inside-page-00-banner__former">
                <div style={{ textShadow: '2px 3px 5px #4d4d4d' }} className="inside-page-00-banner__title inside-page-00-banner__title--main">熱門活動</div>
                <div style={{ textShadow: '2px 3px 5px #4d4d4d' }} className="inside-page-00-banner__title inside-page-00-banner__title--vice">{`共${promoState.totalCount}則活動`}</div>
              </div>
        ...
        <div className="inside-page-00-showcase__latter">
          <div className="card-collection-04">
            <div className="card-collection-04__inner">
              {promoState.list.length > 0 && promoState.list.map((it, index) => (
            ...
            <div className="card-collection-04__pagination pagination-00">
              ...
              <div className="pagination-00__pages">
                {[...Array(Math.ceil(promoState.totalCount / PAGE_SIZE))].map((_, i) => {
                  return (
