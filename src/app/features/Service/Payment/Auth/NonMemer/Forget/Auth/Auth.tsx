...
import { fetchApplyNoAction, fetchPaymentSingleDoneAction, resetPaymentSingleAction, setNonMemberLoginForgetCurrentStepAction } from 'app/store/service/payment/actions';
...
const Auth: React.FC = () => {
  ...
  const forgetState = useSelector((state: RootState) => state.service.payment.authentication.nonMember.login.forget);
  ...
  const formik = useFormik<AuthFormValues>({
    ...
    onSubmit: async (values: AuthFormValues) => {
      ...
      if (!applyNo || !insuredId) return;
      // 清除保單 (非會員-忘記受理編號)
      reduxDispatch(resetPaymentSingleAction());
      // 查詢保單 (非會員-忘記受理編號)
      ...
      if (response) {
        ...
        routerHistory.push({ pathname: ROUTES.SERVICE__PAYMENT__LIST, state: { id: insuredId, applyNo } });
      }
    }
  });
  ...
  useEffect(() => {
    ...
    return () => {
      if (routerHistory.location.pathname.includes(ROUTES.SERVICE__PAYMENT__LIST)) {
